...
class Tile {
  ...
  /**
     * set coordinates of the tile matrix
     * (each block have more pixels. Each pixel have 4 RGB color (R G B alpha))
     * @param col
     * @param ctxTile
     * @returns {Tile}
     */
  ...
  setAvgColor() {
    ...
    function getAvgColor() {
      ...
      return tileSum.map(
        (avgComponent: number) => avgComponent / numPixels || 0,
      );
    }
...
const start = (data: OnMessageDataType) => {
  ...
  for (let row = 0; row < numRow; row += 1) {
    ...
    tileRows[row] = [];
    // extract pixels data foreach row. It's a matrix
    dataPixels = dataCtx.subarray(startPixel, endPixel);
